...
For Dark + Original, the exact semantic danger values are `dangerSurface #4a1f1f`, `dangerBorder #5a2a2a`, standalone `dangerText #f4b8b8` and `onDangerSurface #f2f2f2`. Light defines a scheme-appropriate `onDangerSurface` with adequate contrast against its Light `dangerSurface`. The exact structural TitleGridCard values are `imageOverlay rgba(11, 11, 11, 0.78)` for the title scrim, `imageOverlayMedium rgba(11, 11, 11, 0.82)` for the type badge and `imageOverlayStrong rgba(11, 11, 11, 0.90)` for the pin. Light and Dark may share these structural values because they protect content over images, not general app surfaces. The inventoried `tagLabelOverlay rgba(11, 11, 11, 0.94)` remains unresolved until the Etiquetas migration; no additional overlay token is introduced preventively.
