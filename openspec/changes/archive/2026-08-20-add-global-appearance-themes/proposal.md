...
- Revisar selectivamente `app.json` durante Apply porque `userInterfaceStyle: "dark"` impide el seguimiento completo de System. Cambiarlo a `automatic` e instalar exclusivamente `expo-system-ui` mediante la versión compatible con Expo SDK 54 para habilitar correctamente System en Android native, registrando su config plugin sólo si la configuración Expo/CNG real lo requiere. Preservar los hunks personales protegidos de `app.json` y `package.json`; no se autoriza ninguna otra dependencia.
